...
class EncodecModel(nn.Module):
    ...
    def get_latent_embedding(self, encoded_frame: EncodedFrame) -> torch.Tensor:
        codes, scale = encoded_frame[0]
        codes = codes.transpose(0, 1)
        emb = self.quantizer.decode(codes)
        return emb
    ...
    def encodec_model_24khz(pretrained: bool = True, repository: tp.Optional[Path] = None):
        ...
        if pretrained:
            ...
            model.load_state_dict(torch.load("/apdcephfs_cq8/private_sedricksong/model/encodec_24khz-d7cc33bc.th"))
        model.eval()
        return model
