...
def get_2d_sincos_pos_embed(
    embed_dim, grid_size, cls_token=False, extra_tokens=0, interpolation_scale_h=1.0, interpolation_scale_w=1.0, base_size=(16,16)
):
    """
    ...
    grid_h = np.arange(grid_size[0], dtype=np.float32) / (grid_size[0] / base_size[0]) / interpolation_scale_h
    grid_w = np.arange(grid_size[1], dtype=np.float32) / (grid_size[1] / base_size[1]) / interpolation_scale_w
    #print("22222222", grid_h, grid_w, base_size, interpolation_scale_h, interpolation_scale_w)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        pos_embed = get_2d_sincos_pos_embed(
            embed_dim, int(num_patches**0.5), base_size=self.base_size, interpolation_scale_h=self.interpolation_scale_h, interpolation_scale_w=self.interpolation_scale_w)
        self.register_buffer("pos_embed", torch.from_numpy(pos_embed).float().unsqueeze(0), persistent=False)

    def forward(self, latent):
        height, width = latent.shape[-2] // self.patch_size, latent.shape[-1] // self.patch_size

        #print("latent", latent.size(), self.embed_dim)
        ...
        #print("latent", latent.size(), self.pos_embed.size())
        ...
        if self.height == height or self.width == width:
            pos_embed = get_2d_sincos_pos_embed(
                ...
                interpolation_scale_h=self.interpolation_scale_h,
                interpolation_scale_w=self.interpolation_scale_w
            )
